...
intr_get_level (void) 
{
  ...
  /* Push the flags register on the processor stack, then pop the
     ...
     Interrupts". */
  /*把标志寄存器中的东西存到处理器的栈上 然后
	把值pop到flags上 通过判断flags来返回当前中断状态
  */
  asm volatile ("pushfl; popl %0" : "=g" (flags));

  return flags & FLAG_IF ? INTR_ON : INTR_OFF; // FLAG_IF 是中断标志位 在flags.h中定义
}
...
intr_disable (void) 
{
  enum intr_level old_level = intr_get_level ();  // 获取当前中断状态
  ...
  return old_level;			// 返回获取到的之前的中断状态
}

...
intr_context (void) 
{
  return in_external_intr;  // 返回外中断的标志
}
