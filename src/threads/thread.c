...
{
  struct thread *cur = running_thread ();   // 获得当前运行的线程
  
  ASSERT (intr_get_level () == INTR_OFF);   // 确保中断关闭

  /* Mark us as running. */
  cur->status = THREAD_RUNNING;				// 将当前运行的线程的状态转换为THREAD_RUNNING

  /* Start new time slice. */
  thread_ticks = 0;							// 为新执行的线程重新开始计算时间片
  ...
  if (prev != NULL && prev->status == THREAD_DYING && prev != initial_thread) 
    {
      ASSERT (prev != cur);			// 如果被切换的线程已经dying，那么释放这个线程的资源
      palloc_free_page (prev);		// 但是不能释放初始线程的资源
    }
}
...
schedule (void) 
{
  ...
  ASSERT (intr_get_level () == INTR_OFF);		// 中断必须是关着的
  ASSERT (cur->status != THREAD_RUNNING);		// 当前进程的状态必须已经改变了
  ASSERT (is_thread (next));					// next必须是一个进程

  if (cur != next)			// 如果当前线程与下一个线程 不是同一个线程的话
    prev = switch_threads (cur, next);	// 进行线程切换
...
thread_mlfqs_update_load_avg_and_recent_cpu (void)
{
  ...
  {
    ready_threads++;   // 这里的意思应该是吧当前线程也算成ready_threads了
  }
